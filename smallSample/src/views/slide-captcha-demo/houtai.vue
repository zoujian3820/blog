<template>
  <div class="page">
    <h2 class="header">使用后端校验滑动验证码，兼容电脑和移动端</h2>
    <img class="tu_jie" src="./tu_jie_mobile.png" alt="">
    <p style="padding: 15px 20px 5px;text-align: center;">
      使用后端校验的话，后端生成带缺口图片，前端接收图片及缺口宽高与top值，以及验证码的唯一标识（后台可生成一个uuid），前端生成图片滑块和滑动按钮，<br />
      然后将滑块的坐标（X、Y轴偏移量）及速度（或时间差）传给后端，后端验证坐标是否正确（考率偏移量如X轴： x > -5 && x < 5 ）速度是否正常，返回结果。 </p>
        <div class="container">
...
import useSlideCaptcha from './useSlideCaptchaHoutai';
...
onMounted(() => {
  ...
  const upCallbackFn = async (moveX: number, diffTime: number) => {
    const res = await fetch('/getCaptcha', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ moveX, diffTime })
    })
